/**
 * @file parser.h
 * @brief Recursive-descent parser: transforms the token stream into an AST.
...
typedef struct s_parser {
  t_list *tokens;  /**< `t_list*` of `t_token*` produced by the lexer. */
  t_list *current; /**< List node the parser is currently examining. */
  char   *error;   /**< Human-readable error string (set on syntax error,
                        freed by parser). */
} t_parser;
 ...
 * @details The token list is NOT freed here, caller frees it with
 * lexer_free_tokens().
 */
